...
[TIP]
====
You don't have to build it. Each release ships a ready-to-use Confluent component
archive (`rmoff-kafka-connect-ais-<version>.zip`) on the
https://github.com/rmoff/kafka-connect-ais/releases[Releases page]. To deploy to
Confluent Cloud you can skip Maven entirely and just upload that ZIP --- see
<<deploy-to-confluent-cloud-custom-connectors>>. The local Docker walkthrough below
builds from source.
====
...
confluent connect custom-plugin create kafka-connect-ais \
  --plugin-file rmoff-kafka-connect-ais-0.2.1.zip \
...
The build produces two usable artifacts (where `<version>` matches the project version, e.g. `0.2.1`):
