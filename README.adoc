...
confluent connect custom-plugin create kafka-connect-ais \
  --plugin-file rmoff-kafka-connect-ais-0.3.0.zip \
...
[NOTE]
====
*One connection per source IP.* In testing against the Norwegian Coastal
Administration feed, opening *multiple simultaneous connections from the same
source IP* caused the connections to compete: they were repeatedly cycled and
starved, and overall throughput collapsed — while a *single* connection from
that IP stayed stable and streamed steadily. A single TCP connection per source
IP avoided it entirely.

Practically: keep `tasks.max=1` per host, and avoid running several connectors
(or other AIS clients) behind a shared egress IP/NAT pointing at the same feed.
The connector logs a WARN if it is configured to open more than one connection.
This behaviour may be specific to this particular feed; treat it as something to
be aware of rather than a universal rule.
====
...
* `fragmentBuffer` -- incomplete fragments currently held in memory (log heartbeat only)
* `uptimeMs` -- milliseconds since the current TCP connection epoch (log heartbeat only)

On self-managed Kafka Connect workers with JMX enabled, the five cumulative counters (`emitted`, `decodeErrors`, `incompleteFragments`, `unsupportedTypes`, `reconnects`) are also exposed via JMX at ObjectName `net.rmoff.connect.ais:type=TaskMetrics,host=<host>,port=<port>`. The `fragmentBuffer` and `uptimeMs` values appear in the log heartbeat only — they are not on the MBean. The log heartbeat is the primary observability surface on runtimes without JMX access (e.g. Confluent Cloud Custom Connectors).
...
The build produces two usable artifacts (where `<version>` matches the project version, e.g. `0.3.0`):
